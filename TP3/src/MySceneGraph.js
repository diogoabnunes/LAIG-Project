...
class MySceneGraph {
    ...
    constructor(filename, scene) {
        ...
        this.textTexture = new CGFtexture(this.scene, "./scenes/images/text.png");
    ...
    onXMLReady() {
        ...
        if (!this.changeScene) this.scene.onGraphLoaded();
    }
    ...
    parseSpritesheets(spritesheetsNode){
        ...
        var allSpritesheets=spritesheetsNode.children;
        for(var i=0; i<allSpritesheets.length;i++){
            if (allSpritesheets[i].nodeName == "spritesheet") {
                var spritesheetID = this.reader.getString(allSpritesheets[i],'id');
                if (spritesheetID == null) return "spritesheet ID null";
                if (this.spritesheets[spritesheetID] != null) return "spritesheet ID not unique";
                
                var path= this.reader.getString(allSpritesheets[i],'path');
                if(path==null) return"spritesheet path null";
                var texture = new CGFtexture(this.scene, path);

                var sizeM=this.reader.getFloat(allSpritesheets[i],'sizeM');
                var sizeN=this.reader.getFloat(allSpritesheets[i],'sizeN');

                var spritesheet= new MySpriteSheet(this.scene,spritesheetID,texture,sizeM,sizeN);
    ...
    parseAnimations(animationsNode){
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            var animation = new KeyframeAnimation(animationID, this.scene);
            ...
            for (var j = 0; j < grandChildren.length; j++) {
                ...
                var keyframeChildren = grandChildren[j].children;

                var translation = keyframeChildren[0];
                var tx = this.reader.getFloat(translation, 'x');
                if (isNaN(tx)) return "translation X is NaN";
                var ty = this.reader.getFloat(translation, 'y');
                if (isNaN(ty)) return "translation Y is NaN";
                var tz = this.reader.getFloat(translation, 'z');
                if (isNaN(tz)) return "translation Z is NaN";

                //rotation axis=X;
                var rotation1 = keyframeChildren[1];
                var angle1 = this.reader.getFloat(rotation1, 'angle');

                //rotation axis=Y;
                var rotation2 = keyframeChildren[2];
                var angle2 = this.reader.getFloat(rotation2, 'angle');
                
                 //rotation axis=Z;
                var rotation3 = keyframeChildren[3];
                var angle3 = this.reader.getFloat(rotation3, 'angle');

                var scale = keyframeChildren[4];
                var sx = this.reader.getFloat(scale, 'sx');
                if (isNaN(tx)) return "scale X is NaN";
                var sy = this.reader.getFloat(scale, 'sy');
                if (isNaN(ty)) return "scale Y is NaN";
                var sz = this.reader.getFloat(scale, 'sz');
                if (isNaN(tz)) return "scale Z is NaN";

                var keyframe = new Keyframe(keyframeInstant,[tx,ty,tz],[angle1,angle2,angle3],[sx,sy,sz]);
  ...
  parseNodes(nodesNode) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            this.nodes[nodeID] = new MyNode(nodeID, this.scene);
            ...
            if (animationsIndex != -1) {
                var animationID = this.reader.getString(nodeElements[animationsIndex], 'id');
                if (animationID == null) return "no animation specified"; 
                if (animationID != "null" && animationID == -1) {
                    return "no animation with that ID";
                }
            ...
            if (nodeElements[descendantsIndex]) {
                ...
                for (var x = 0; x < descendantElements.length; x++) {
                    if (descendantElements[x].nodeName == "noderef") {
                        ...
                        var leaf=this.parseLeafs(descendantElements[x], this.textTexture);
    ...
    parseLeafs(leafsNode, texture) {
        ...
        if (this.reader.getString(leafsNode, 'type') == "triangle") {
            ...
            leaf = new MyLeaf(this.scene,new MyTriangle(this.scene, x1, y1, x2, y2, x3, y3)); 
        }
            ...
            leaf = new MyLeaf(this.scene,new MyTriangleTwoSided(this.scene, x1, y1, x2, y2, x3, y3)); 
        }
            ...
            leaf = new MyLeaf(this.scene,new MyRectangle(this.scene, x1, y1, x2, y2));     
        }
            ...
            leaf = new MyLeaf(this.scene,new MySphere(this.scene, radius, slices, stacks));
        }
            ...
            leaf = new MyLeaf(this.scene,new MyCylinder(this.scene, height, topRadius, bottomRadius, stacks, slices));
        }
            ...
            leaf =new MyLeaf(this.scene,new MyTorus(this.scene, inner, outer, slices, loops));
        }

        else if (this.reader.getString(leafsNode, 'type') == "spriteanim") {
            var ssid = this.reader.getString(leafsNode,'ssid');
            var startCell= this.reader.getInteger(leafsNode,'startCell');
            var endCell= this.reader.getInteger(leafsNode,'endCell');
            var duration= this.reader.getFloat(leafsNode,'duration');
            
            var spriteAnim = new MySpriteAnimation(this.spritesheets[ssid],duration,startCell,endCell);
            leaf=new MyLeaf(this.scene, spriteAnim);
            this.spriteAnimations.push(spriteAnim);
        }

        else if (this.reader.getString(leafsNode, 'type') == "plane") {
            var nPartsU = this.reader.getInteger(leafsNode, 'npartsU');
            var nPartsV = this.reader.getInteger(leafsNode, 'npartsV');
            
            leaf = new MyLeaf(this.scene, new MyPlane(this.scene, nPartsU, nPartsV));
        }
        
        else if(this.reader.getString(leafsNode, 'type') == "spritetext"){
            var text= this.reader.getString(leafsNode,'text');
            leaf=new MySpriteText(this.scene,text, texture);
        }

        else if (this.reader.getString(leafsNode, 'type') == "patch") {
            var nPointsU = this.reader.getInteger(leafsNode, 'npointsU');
            var nPointsV = this.reader.getInteger(leafsNode, 'npointsV');
            var nPartsU = this.reader.getInteger(leafsNode, 'npartsU');
            var nPartsV = this.reader.getInteger(leafsNode, 'npartsV');

            var controlPoints = [];

            var children = leafsNode.children;
            for (var i = 0; i < children.length; i++) {
                var x = this.reader.getFloat(children[i], 'x');
                var y = this.reader.getFloat(children[i], 'y');
                var z = this.reader.getFloat(children[i], 'z');
                controlPoints.push([x,y,z]);
            }
            if (children.length != (nPointsU * nPointsV)) return "incorrect number of control points";
            leaf = new MyLeaf(this.scene, new MyPatch(this.scene, nPointsU, nPointsV, nPartsU, nPartsV, controlPoints));
        }

        else if (this.reader.getString(leafsNode, 'type') == "defbarrel") {
            var base = this.reader.getFloat(leafsNode, 'base');
            var middle = this.reader.getFloat(leafsNode, 'middle');
            var height = this.reader.getFloat(leafsNode, 'height');
            var slices = this.reader.getInteger(leafsNode, 'slices');
            var stacks = this.reader.getInteger(leafsNode, 'stacks');

            leaf = new MyLeaf(this.scene, new MyBarrel(this.scene, base, middle, height, slices, stacks));
        }
    ...
    displayScene() {
        ...
        thisNode.display(this.nodes, this.materialStack,this.textureStack);
    }
}